...
func TestPayloadFields(t *testing.T) {
	...
	for i, p := range payloads {
		...
		if p.Marker == "" {
			t.Errorf("Payload %d has empty Marker", i)
		}
	}
}
...
func TestCorePayloadsPresent(t *testing.T) {
	...
	// Verify email header types are present
	...
	for _, expected := range expectedMarkers {
		...
		if !found {
			t.Errorf("Expected payload containing header %q not found", expected)
		}
	}
}
...
func TestPayloadMarkers(t *testing.T) {
	...
	for _, p := range payloads {
		if p.Marker == "" {
			t.Errorf("Payload has empty Marker: %s", truncate(p.Value, 30))
		}
	}
}
